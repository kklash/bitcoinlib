...
func (kf *KeyCompressionFixture) TestCompress(t *testing.T) {
	...
	if _, _, err := DeserializePoint(uncompressed); err != nil {
...
func (kf *KeyCompressionFixture) TestUncompress(t *testing.T) {
	...
	if !IsCompressedPublicKey(compressed) {
		...
		return
	} else if _, _, err := DeserializePoint(compressed); err != nil {
		t.Errorf("Expected public key %x to be valid", compressed)
		return
	}

	uncompressed, err := UncompressPublicKey(compressed)
	if err != nil {
		t.Errorf("Failed to uncompress public key %x\nError: %s", compressed, err)
		return
	} else if uncompressedHex := hex.EncodeToString(uncompressed); uncompressedHex != kf.uncompressedHex {
...
func TestPublicKeyCompression(t *testing.T) {
	fixtures := []*KeyCompressionFixture{
		...
		},
		{
			...
			"0454236f7d1124fc07600ad3eec5ac47393bf963fbf0608bcce255e685580d16d92b266380164d9b76f0f442d0a618314ae40a463e0ca69f68d40d572c21f8927e",
		},
		{
		...
		},
		{
			...
			"04098190f2263db5049e659448343683a77efcfe5162b445dc491dcf19c34ecccda31663c11d630c9c4c5cf14997ea7df9f774082829fd373ca9be4a4739e44ba8",
		},
		{
			"022f5379552d5f68b83747057a901ca7b8846337683236e6055ed2ce5f72230503",
			"042f5379552d5f68b83747057a901ca7b8846337683236e6055ed2ce5f7223050325ac711e204d5ef4349f2eb18ba03fdafb7b9a830159ca2c05c08c3359dcfff4",
		},
	}
